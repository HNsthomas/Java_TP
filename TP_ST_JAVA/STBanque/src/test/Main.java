...
public class Main {
	...
	public static void main(String[] args) {	
		...
		System.out.println("\n Liste des comptes triées!");
		classerCompte(collectionCompte);
	...
	public static Hashtable getHashtable(Compte[] comptes) {
		...
		Hashtable <Integer, Compte> tab = new Hashtable<Integer, Compte>();
		Integer i = 0;
	...
	public static void classerCompte(Compte[] comptes) {
		...
		Stream<Compte> sp = tab.stream();
		List<Compte> lp = sp.sorted(Comparator.comparing(Compte::getSolde)).collect(Collectors.toList());
	...
	public static Flux[] addFluxInTab(String fluxType,String comment, double mount, int numeroCompteCible, Flux[] flux) {
		...
		if(fluxType == "debit")
			tab[i] = new Debit(comment, i, mount, numeroCompteCible, true, currentTime);
		else if(fluxType == "credit")
			tab[i] = new Credit(comment, i, mount, numeroCompteCible, true, currentTime);
		return tab;
	}
	public static Flux[] addFluxInTab(String fluxType,String comment, double mount, int numeroCompteCible, Flux[] flux, int numeroCompteEmetteur) {
		...
		if(fluxType.equals("virement")) {
			tab[i] = new Virement(comment, i, mount, numeroCompteCible, true, currentTime, numeroCompteEmetteur);
